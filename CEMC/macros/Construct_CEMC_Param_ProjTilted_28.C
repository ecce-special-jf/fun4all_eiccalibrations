...
four_corner * get_block(const int block_id, const int z_sign);
...
void Construct_CEMC_Param_ProjTilted()
{
  ...
  cout << "loaded libs" << endl;

  PHParameters *param = new PHParameters("CEMC_0");

  cout << "made params" << endl;
  ...
  cout << " JFJF made it to block level" << endl;
    ...
    for (int block_id = 0; block_id < 0 + nblock; block_id++)
    {
      for (int block_sec = 0; block_sec < max_phi_bin_in_sec; block_sec++)
      {
        ...
        if (1)
        {
          ...
          AveragePlaneNorm *= 1.0 / AveragePlaneNorm.Mag();
  ...
  TFile *f = TFile::Open("thisFile.xml", "recreate");
  ...
  */
  ...
  paramcontainer->Write("PdbParameterMapContainer");
  delete f;
  ...
  const double data[] =
      {
          ...
          107.9138855, -152.3892519,
          103.99111,   -156.19695,
          97.62202869, -141.320895,
          93.87578546, -144.832955,
          107.9138855, -159.3892519,
          103.98111,   -163.39695,
          97.71202869, -147.3820895,
          93.87578546, -150.932955,
          107.9138855, -166.3892519,
          103.97211,   -170.39695,
          97.81102869, -153.3820895,
          93.87578546, -156.932955,
          107.9138855, -173.3892519,
          103.96311,   -177.29695,
          97.9102869,  -159.820895,
          93.87578546, -162.932955,
          0, 0, 0, 0, 0};
